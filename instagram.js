...
const instagram = {
  ...
  initialize: async (lang) => {
    instagram.buttonStrings =
      lang == "en" || lang == "en-US" ? engStrings : espStrings;
  ...
  login: async (username, password) => {
    ...
    let loginButton = await instagram.page.$x(
      `//div[contains(text(), "${instagram.buttonStrings[0]}")]`
    );
    ...
    await instagram.page.waitFor(
      `a > svg[aria-label="${instagram.buttonStrings[1]}"]`
    );
  },
  ...
  likearALV: async (tags = [], cantPost) => {
    ...
    for (let tag of tags) {
      try {
        ...
        let postImages = await instagram.page.$$(
        ...
        for (let i = 0; i < cantPost; i++) {
          ...
          try {
            ...
            /* Like to the post */
            let isLikeable = await instagram.page.$(
              `svg[aria-label="${instagram.buttonStrings[2]}"]`
            );
            if (isLikeable) {
        ...
        console.log(
          `Hubo un error al cargar el tag: ${tag}, pasemos al siguiente`
        );
        instagram.errores++;
      }
